...
## 7. Presentation notes (demo script)

Suggested talking points for the "Inventory management and low-stock
alerts" section of the group presentation:

1. **Open the Materials Management screen** and point out the three
   panels: form, table, search bar.
2. **Add a material** with a small quantity and a higher reorder level
   (e.g. quantity 2, reorder level 5) → show the row turning red and the
   Status column reading `LOW STOCK` the moment it's saved.
3. **Try to save a negative quantity** (e.g. type `-5`) → show the
   validation message, and explain it's blocked both in the form and
   again in the DAO before any SQL runs.
4. **Use the search box** to filter by name, then tick **"Show low-stock
   only"** to demonstrate search and filtering together.
5. **Update** the same material's quantity above the reorder level →
   show the row returning to white/`OK`.
6. **Delete** a material and explain the soft-delete decision: it stays
   out of active lists but its issuance history is preserved, because of
   the `ON DELETE RESTRICT` relationship to `stock_issuances`.
7. Close by connecting this back to the bigger system: the same
   `quantity <= reorder_level` rule that colours this table also powers
   the Dashboard's low-stock count and the Low-Stock report, so the
   business rule only has to be correct in one place.
